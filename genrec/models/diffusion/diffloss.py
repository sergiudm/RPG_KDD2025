...
class DiffLoss(nn.Module):
    """Diffusion Loss with support for Rectified Flow"""
    ...
    def __init__(
        ...
        # Adjust output channels based on diffusion type
        if use_rectified_flow:
            # For rectified flow, we only predict velocity
            out_channels = target_channels
        else:
            # For gaussian diffusion, we predict epsilon and variance
        ...
        # Initialize diffusion based on type
        if use_rectified_flow:
            ...
            self.gen_diffusion = create_diffusion(
                timestep_respacing=str(num_sampling_steps), noise_schedule="cosine"
            )
    ...
    def forward(self, target, z, mask=None):
        if self.use_rectified_flow:
            ...
            # Original gaussian diffusion training
            t = torch.randint(
                0,
                self.train_diffusion.num_timesteps,
                (target.shape[0],),
...
class SimpleMLPAdaLN(nn.Module):
    ...
    def forward_with_cfg(self, x, t, c, cfg_scale):
        ...
        if self.use_rectified_flow:
            # For rectified flow, apply guidance to velocity
            cond_velocity, uncond_velocity = torch.split(
                model_out, len(model_out) // 2, dim=0
            )
            half_velocity = uncond_velocity + cfg_scale * (
                cond_velocity - uncond_velocity
            )
            return torch.cat([half_velocity, half_velocity], dim=0)
        else:
            # For gaussian diffusion, apply guidance to epsilon and variance
            eps, rest = (
